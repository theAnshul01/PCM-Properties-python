...
label = CTkLabel(master=app, text="Ambient temperature ")
label.grid(row = 0, column = 0, columnspan = 3)

Atemp = CTkEntry(master=app, placeholder_text=" temperature 25°C ")
Atemp.grid(row = 1, column = 0, columnspan = 3)

# Place the frames in the grid layout
frame1 = CTkFrame(master=app, fg_color="#d6d6a9")
frame2 = CTkFrame(master=app, fg_color="#9be8bb") 
frame1.grid(row=2, column=0,  padx=10, pady=10)  # Fill the entire column
frame2.grid(row=2, column=1, padx=10, pady=10)  # Fill the entire column
...
frame1Title.grid(row=0, column = 0, columnspan = 2, padx=20, pady=10)
# excel filepath - PCM
excelFilePath = CTkLabel(master=frame1, text="Excel File Path: ", text_color="black")
excelFilePath.grid(row=1, column = 0, columnspan = 1, padx = 10, pady = 10)
fPath = CTkEntry(master=frame1, placeholder_text="file path")
fPath.grid(row = 1, column = 1, columnspan = 1, pady=10)
# x col name
xAxisCol = CTkLabel(master=frame1, text="X-axis column name ", text_color="black")
xAxisCol.grid(row=2, column = 0, columnspan = 1, padx = 10, pady = 10)
xCol = CTkEntry(master=frame1, placeholder_text="x col name")
xCol.grid(row = 2, column = 1, columnspan = 1, pady=10)
# y col name
yAxisCol = CTkLabel(master=frame1, text="Y-axis column name ", text_color="black")
yAxisCol.grid(row=3, column = 0, columnspan = 1, padx = 10, pady = 10)
yCol = CTkEntry(master=frame1, placeholder_text="y col name")
yCol.grid(row = 3, column = 1, columnspan = 1, pady=10)

btn1 = CTkButton(master=frame1, text="Plot Graph", corner_radius=20, fg_color="#C850C0", hover_color="#4158D0", command=plotcall)
btn1.grid(row = 2, column =2, columnspan = 1, pady=10)

# PCM - starting point 3 times and ending point 3 times
nestedFrame1 = CTkFrame(master=frame1, fg_color="#e3a6da")
nestedFrame1.grid(row = 5, column = 0, columnspan = 4, padx = 10, pady = 10)
...
sPoint2.grid(row = 1, column = 0, columnspan = 2, padx=10)
sPt2 = CTkEntry(master=nestedFrame1, placeholder_text="Enter st point 2")
sPt2.grid(row= 1, column=2, padx=10)
...
ePoint3.grid(row = 2, column = 3, columnspan = 2, padx=10 )
ePt3 = CTkEntry(master=nestedFrame1, placeholder_text="Ending point 3")
ePt3.grid(row = 2, column=5, padx=10)
...
frame2Title.grid(row=0, column = 0, columnspan = 3, padx=10, pady=10)
...
fPathRef.grid(row = 1, column = 1, columnspan = 1, padx = 10, pady=10)
...
xColRef.grid(row = 2, column = 1, columnspan = 1, padx = 10, pady=10)
# y col name
...
yColRef = CTkEntry(master=frame2, placeholder_text="y col name")
yColRef.grid(row = 3, column = 1, columnspan = 1, padx = 10, pady=10)
...
def main():
    ...
    T_init = float(PCM_temp.get())
    T_m1 = float(Tm1_temp.get())

    if(water_weight and TT_Weight and Cp_t and PCM_Weight and T_init and T_m1):
        # # Cp in solid state
        ...
        # Enthalpy of PCM
        T_m2 = T_m1 - 6
        Hm = ((((water_weight*4.18) + (TT_Weight*Cp_t))/PCM_Weight) *A2/A1Ref) * (T_init - T_m1) - (TT_Weight*Cp_t*(T_m1-T_m2))/PCM_Weight

        cp_solid_state.configure(text=f"Cps: {Cp_s:.3f}")
        cp_liquid_state.configure(text=f"Cpl: {Cp_l:.3f}")
        enthalpy.configure(text=f"Hm: {Hm:.3f}")
    else:
...
testTube.grid(row = 1, column = 1, columnspan = 1, padx = 10, pady = 10)
...
pcmWeight.grid(row = 2, column = 1, columnspan = 1, padx = 10, pady = 10)
...
water_Weight.grid(row = 3, column = 1, columnspan = 1, padx = 10, pady = 10)

cpTestTube = CTkLabel(master=frame3, text = "Cp of test tube", text_color="black" )
cpTestTube.grid(row = 1, column = 2, padx = 10, pady = 10)
cpT = CTkEntry(master=frame3, placeholder_text="Enter test tube Cp")
cpT.grid(row = 1, column = 3, columnspan = 1, padx = 10, pady = 10)

temperature = CTkLabel(master=frame3, text = "Initial temperature of PCM", text_color="black" )
temperature.grid(row = 2, column = 2, padx = 10, pady = 10)
PCM_temp = CTkEntry(master=frame3, placeholder_text="Initial PCM temp")
PCM_temp.grid(row = 2, column = 3, columnspan = 1, padx = 10, pady = 10)

Tm1 = CTkLabel(master=frame3, text = "Tm1 temperature", text_color="black" )
Tm1.grid(row = 3, column = 2, padx = 10, pady = 10)
Tm1_temp = CTkEntry(master=frame3, placeholder_text="Tm1 temperature")
Tm1_temp.grid(row = 3, column = 3, columnspan = 1, padx = 10, pady = 10)

btn3 = CTkButton(master=frame3, text="Calculate", corner_radius=20, fg_color="#C850C0", hover_color="#4158D0", command=main)
btn3.grid(row = 2, column = 5, padx=10, pady=10)

# results 
cp_solid_state = CTkLabel(master=frame3, text="")
cp_solid_state.grid(row=4, column=1, columnspan=1, padx=10, pady=10)
cp_liquid_state = CTkLabel(master=frame3, text="")
cp_liquid_state.grid(row=4, column=2, columnspan=1, padx=10, pady=10)
